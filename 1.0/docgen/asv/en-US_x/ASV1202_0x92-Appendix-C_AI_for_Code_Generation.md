...
This appendix lists organizational controls for using AI coding tools safely. The range spans from baseline to advanced. The scope includes coding, code review, and the rest of the SSDLC.

Treat the AI coding agent as an actor in the supply chain, not as a passive helper. It has an identity, authority, and the ability to act on its own behalf or to be acted upon by an attacker.

Two things follow from that framing.

Your own agent can be turned against you. A code-review bot that reads a malicious pull request description can be prompt-injected into approving the same code it was intended to reject. The injection does not have to be subtle—it just has to reach the bot’s context window.
...
Before you begin verifying against Appendix C, the hosting environment must meet this baseline:

* OWASP ASVS v5 compliance. This appendix supplements the ASVS v5 requirements for coding quality and CI/CD deployment security (V10 above all). It does not replace them. Coverage of CI/CD pipeline security under ASVS v5 must be in place before Appendix C is verified. The same applies to coverage of the OWASP Top 10 CI/CD Security Risks.
* SLSA implementation. On the core integration and delivery lines, implement SLSA Build Track Level 2 or higher, with provenance generated and verified for release artifacts. If production AI agents act on source repositories, also adopt the SLSA Source Track (v1.2) for authorship and review controls.
* Platform control invariants. Branch protection or ruleset constraints on release paths. At a minimum: no unreviewed merges into default or release branches, signed commits where the platform supports them, required status checks, protected environments, and a trail for every human override.

---

## AC.1 AI-Assisted Secure-Coding Workflow

AI tooling has to fit into the existing secure software development lifecycle (SSDLC) without weakening any of the security gates already in place. Equally important: document adversarial-AI threat scenarios that justify each guardrail. Doing this up front is much easier than reconstructing it after the fact.

<!-- markdownlint-disable MD013 -->
| #      | Description                                                                                                                                                                                                                                                                                                                | Level |
| ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- |
| AC.1.1 | Verify that the documented workflow specifies when AI tools may generate, refactor, or review code. The workflow names the approved tools, prohibited use cases, and the data classifications that are permitted as input.                                                                                                 | 1     |
| AC.1.2 | Verify that the workflow covers every SSDLC phase, from design and implementation through code review, testing, deployment, and post-deployment monitoring, and that it names the security gates that remain mandatory regardless of whether AI was involved or not.                                                       | 2     |
| AC.1.3 | Verify that the workflow names the adversarial-AI threat scenarios it is designed to mitigate. The list should cover prompt injection delivered through PR content, AI-generated supply-chain payloads, autonomous agents approving their own work, fork-PR secret exfiltration, and compromise of the model supply chain. | 2     |
| AC.1.4 | Verify that metrics are collected for AI-produced and AI-mediated code, and that the results are compared against a human-only baseline. Vulnerability density, mean time to detect, AI-attributable defect rate, prompt-injection detection rate, and fork-PR rejection rate are all useful.                              | 3     |
...
Do not adopt an AI coding tool until it has been evaluated. In particular, examine three areas: its security capabilities, its resistance to adversarial input, and the risk inherited from its supply chain.
...
| AC.2.2 | Verify that the evaluation of each tool covers the local components (static and dynamic analysis), the SaaS endpoints (TLS, AuthN/AuthZ, logging, data residency), and the vendor's model supply chain (training-data provenance, fine-tune history, RAG sources). Each of these is reviewed, and the review is documented.         | 2     |
| AC.2.3 | Verify that each tool undergoes adversarial robustness testing before onboarding. The testing is repeated after any material change to the model or to the system prompts. Coverage includes automated prompt-injection probes, jailbreak suites, and indirect-injection corpora delivered through realistic PR and issue surfaces. | 2     |
| AC.2.4 | Verify that evaluations follow a recognized framework, such as the NIST AI RMF, the NIST AI 600-1 Generative AI Profile, or ISO/IEC 42001. Perform evaluations again after a major version update, a vendor incident, or new threat intelligence that is relevant to the tool class.                                                | 3     |
...
| #      | Description                                                                                                                                                                                                                                                                                                                           | Level |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- |
| AC.3.1 | Verify that the written guidance prohibits entering secrets, credentials, PII, or classified data into any prompt sent to an AI tool. The guidance is enforced via pre-commit hooks, IDE integrations, and CI.                                                                                                                        | 1     |
| AC.3.2 | Verify that technical controls automatically remove sensitive material from any context window sent to an AI tool. Client-side redaction, approved context filters, and secret scanners with pre-prompt hooks all qualify.                                                                                                            | 1     |
| AC.3.3 | Verify that any externally sourced context provided to an AI tool is treated as untrusted and is screened for prompt injection before it reaches the prompt. Sources to cover: PR descriptions and comments, fork-supplied diffs, issue bodies, commit messages, third-party documentation, web search results, and MCP tool outputs. | 1     |
| AC.3.4 | Verify that the AI tool enforces an instruction hierarchy, with system and developer messages taking precedence over untrusted repository content. This hierarchy must hold across multi-turn conversations and tool-augmented workflows.                                                                                             | 1     |
| AC.3.5 | Verify that input length controls prevent untrusted PR or repository content from crowding out system instructions or safety directives within the effective context window. Oversized inputs are rejected outright. Silent truncation is not acceptable.                                                                             | 2     |
| AC.3.6 | Verify that prompts and AI responses are encrypted in transit and at rest, and retained according to the data-classification policy. Tenants and projects are cryptographically separated from each other.                                                                                                                            | 3     |
...
Catch the vulnerabilities that AI output introduces. Fix them before the code reaches a merge or a deployment.

<!-- markdownlint-disable MD013 -->
| #      | Description                                                                                                                                                                                                                                                                                                                                                       | Level |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- |
| AC.4.1 | Verify that AI-generated code always undergoes code review by a qualified human engineer. The reviewer must not be the same identity that requested the AI generation in the first place (segregation of duties). And the AI agent itself does not count as the human reviewer.                                                                                   | 1     |
| AC.4.2 | Verify that automated security testing runs on every pull request that contains AI-generated code: SAST, IAST, DAST, secret scanning, IaC scanning, and SCA. Where the scanner supports them, AI-attribution-aware rules are enabled.                                                                                                                             | 2     |
...
| #      | Description                                                                                                                                                                                                       | Level |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- |
| AC.5.1 | Verify that prompt-and-response pairs are logged with stable correlation identifiers so that an investigator can later replay the entire chain: prompt → response → commit → build → deployment.                  | 1     |
| AC.5.2 | Verify that developers can access the citations (training snippets, retrieved documents, MCP tool outputs) that support a suggestion, and that the citation chain travels with the artifact.                      | 2     |
| AC.5.3 | Verify that explainability reports, AI event logs, and citation records are stored in tamper-evident storage (append-only, WORM, or an immutable log store) and that they are referenced during security reviews. | 3     |
...
Improve model security over time. Monitor for negative drift. Continue red-teaming the AI tooling. The red-team scope in this family covers the AI tooling itself; the underlying systems and services the tooling depends on are handled by separate programs.
...
Infrastructure code, CI/CD workflow files, deployment manifests, and security policy artifacts all have an outsized impact when they are incorrect. When AI has generated them, validation must be correspondingly stricter than it is for ordinary application code.
...
| AC.7.2 | Verify that AI-generated infrastructure and pipeline configurations require human review and approval before they run in any environment other than a hermetic sandbox.                                                                                                                                                                                                                                                                           | 2     |
...
| AC.7.5 | Verify that drift detection compares the deployed infrastructure and live workflow configurations against signed, AI-attributed baselines, and alerts on any unauthorized modification.                                                                                                                                                                                                                                                           | 3     |
...
Autonomous AI agents that generate code or configuration receive the same separation-of-duties treatment as humans. They cannot approve, merge, or promote their own work. This applies at both the policy layer and the technical layer.
...
| AC.8.3 | Verify that autonomous agents cannot bypass branch protection, required reviews, required status checks, signed-commit requirements, or merge queues. Any attempt by an agent to change these settings triggers a security alert.                                       | 2     |
...
Deployment and promotion pipelines must validate the cryptographic origin and generation history of AI-generated artifacts. They do this before allowing the artifact to proceed.

<!-- markdownlint-disable MD013 -->
| #      | Description                                                                                                                                                                                                                                                             | Level |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- |
| AC.9.1 | Verify that AI-generated artifacts include signed origin and generation metadata (in-toto or SLSA provenance attestations, AI-BOM entries) identifying the AI system that produced them, the generation context, the humans involved, and the associated audit records. | 1     |
...
| AC.9.3 | Verify that artifacts are rejected at deployment and quarantined for review when they are missing required origin and generation information, signed with untrusted keys, or produced by an unapproved AI system or environment.                                        | 3     |
...
AI-generated artifacts need complete and consistent provenance and generation records, validated before integration or deployment. The reason matters. Policy-based enforcement of provenance tracking only works if the recorded information is itself complete and consistent. When records are missing fields, or when the fields they do have contradict each other, detections get missed and enforcement leaves gaps. So provenance tracking is treated as a first-class requirement here and validated before an artifact is accepted.

<!-- markdownlint-disable MD013 -->
| #       | Description                                                                                                                                                                                                                                                                                                                                                 | Level |
| ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- |
| AC.10.1 | Verify that AI-generated artifacts include the required origin and generation fields: model identity and version, tool or agent identity, generation context, prompt hash, human involvement, session identifiers, and correlation IDs.                                                                                                                     | 1     |
| AC.10.2 | Verify that the origin and generation metadata are checked for completeness and consistency: no missing or ambiguous fields, values normalized to a single representation, and a signature chain that validates back to a trusted root.                                                                                                                     | 2     |
| AC.10.3 | Verify that artifacts with incomplete, inconsistent, or unverifiable origin and generation metadata are rejected before merge or deployment, and that the rejection event is logged so trends can be tracked. Rejection occurs on the verifier side, against the attestation or proof model defined in SLSA and the verification criteria in ISO/IEC 42001. | 3     |
...
AI code-review bots, PR-comment bots, MCP-driven assistants (Model Context Protocol), and IDE copilots are all reachable through untrusted repository content. The reachable surfaces include PR diffs, descriptions, comments, issues, and any workflow files supplied from a fork. This category covers the case where an attacker uses one of those surfaces to push a defender’s own AI agent into approving, ignoring, or actively assisting a supply-chain attack.

>Relationship to AISVS C2.1, C9.3, and C9.6: AC.11.1 through AC.11.5 are applications of three AISVS chapter controls to the specific case of AI code-review and assistant bots operating over untrusted PR content. The three chapter controls are C2.1 (Prompt Input Validation), C9.3 (Tool Sandboxing), and C9.6 (Action Authorization). The appendix restates each one with bot-specific guidance. The counting rule is the same as elsewhere: a finding here is either an additional gap that the upstream chapter did not address, or it is already counted under the chapter. Not both.
...
| AC.11.2 | Verify that the AI review and assistant bot system prompts and policy configurations are integrity-checked at load time (signed, hash-pinned), and that nothing in the repository, in the branch contents, in PR-sourced environment variables, or in any other user-controllable input can modify them.                                        | 1     |
| AC.11.3 | Verify that AI review and assistant bots emit only structured, schema-validated output (JSON with an allowlist of fields and actions). Any free-form output is treated as untrusted and is never executed as a command, a query, a shell snippet, or a workflow step.                                                                           | 1     |
...
| AC.11.6 | Verify that AI review and assistant bots log all prompts (including externally sourced context), tool calls, and outputs to tamper-evident storage. Egress patterns (URLs, IPs, DNS, and payload sizes) are continuously monitored for exfiltration indicators, with alerting tuned for webhook, paste-site, and bin-service destinations.      | 2     |
...
## AC.12 Hardening AI Augmentation-Specific CI/CD Pipelines

Two kinds of CI/CD pipeline controls are in scope for this family: those that AI augmentation_newly requires_, and those that AI augmentation_breaks_. Generic CI/CD hygiene is not in scope here; it is covered elsewhere. Short-lived credentials, immutable action pinning, branch protection, SLSA Build Track L3 provenance, and multi-party production approval are all addressed by OWASP ASVS v5 V10, the OWASP Top 10 CI/CD Security Risks (CICD-SEC-01 through CICD-SEC-10), NIST SP 800-204D, and SLSA v1.2. Adopters implement those baselines and verify them against the originating standards. We do not repeat that assessment here.

<!-- markdownlint-disable MD013 -->
| #       | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Level |
| ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- |
| AC.12.1 | Verify that workflows triggered by untrusted contributions (GitHub Actions`pull_request_target`, `workflow_run`, and equivalent fork-aware triggers in other CI systems) never checks out, builds, tests, or otherwise executes untrusted code in a context that has repository write permissions or access to repository, organization, package-registry, cloud, or deployment secrets. When a privileged follow-up step is needed, the untrusted contribution is first processed in an unprivileged`pull_request`workflow, and only validated passive artifacts are passed forward to a separate privileged workflow. | 1     |
...
| AC.12.3 | Verify that secrets are not exposed to workflows that run code from forks or first-time contributors. Environment-protection rules (or the platform equivalent, such as protected variables and deployment approvals) require a manual approval before any secret-bearing job can run for those contributions. This control works with AC.11.7 and AC.13.2. Bot-level enforcement under AC.11.7 does not replace the platform-level enforcement required here.                                                                                                                                                          | 1     |
| AC.12.4 | Verify that self-hosted or persistent runners used by AI tooling are ephemeral (destroyed after each job), network-segmented, and isolated from production credentials. Persistent or long-lived runners must not process fork PRs or AI-generated untrusted artifacts under any circumstances.                                                                                                                                                                                                                                                                                                                         | 2     |
| AC.12.5 | Verify that changes to workflow definition files (`.github/workflows/*`, `.gitlab-ci.yml`, `Jenkinsfile`, Argo, Tekton, and equivalents) are detected on every PR and routed through an elevated review path that includes a security reviewer, regardless of who the contributor is or whether AI was involved. AI agents must not be granted bypass authority over this review path.                                                                                                                                                                                                                                  | 2     |
...
| AC.12.8 | Verify that the remediation of a vulnerable workflow includes invalidating or revalidating any PR that was opened before the fix was released. Without this step, a later commit to the same PR can pick up the stale workflow definition and bypass the fix.                                                                                                                                                                                                                                                                                                                                                           | 2     |
...
The previous families focused on defending your own AI from misuse. This one flips the perspective. Here, the AI is on the attacker’s side, and you’re trying to spot the signal in incoming contributions and content. The scenario worth defending against is one where an attacker uses AI to run fork-and-PR campaigns at scale, with malicious payloads tailored to the target repository.
...
| AC.13.2 | Verify that pull requests (PRs) from first-time or low-reputation contributors require maintainer approval before any privileged workflow processes them. Privileged workflows here include AI review bots, secret-bearing jobs, and external-integration calls.                                                                                                                                                                | 1     |
| AC.13.3 | Verify that automated PR pipeline gates detect known indicators of LLM-generated or LLM-assisted malicious payload patterns: registry-confusable or typosquatted dependency names, package references that do not resolve to any published version, and dependencies whose creation, first-publication, or maintainer-change timestamps appear anomalous relative to the PR.                                                    | 2     |
| AC.13.4 | Verify that the detection rules are mapped to MITRE ATT&CK (T1195 Supply Chain Compromise and CI/CD-relevant sub-techniques) and to MITRE ATLAS techniques, maintained for the inbound contribution analysis use case, and reviewed against current threat intelligence.                                                                                                                                                        | 2     |
...
Things go wrong eventually. When an AI-adjacent compromise (a prompt-injected bot, a leaked CI secret, or a malicious AI-generated artifact in a build) is suspected or confirmed, the goal is to contain the damage and shorten recovery time.
...
| AC.14.2 | Verify that any secret that was used in a workflow run associated with a suspicious PR, a prompt-injection event, or an AI-agent anomaly is automatically rotated, and that downstream issuers (cloud IAM, package registries, and signing-key custodians) are notified of the rotation.                                                                                               | 1     |
| AC.14.3 | Verify that AI agent identities (keys, tokens, OIDC trust grants) can be rapidly revoked and quarantined, with a written target time-to-revoke that is tested at least once per year.                                                                                                                                                                                                  | 2     |
...
| AC.14.5 | Verify that automated remediation is tested in tabletop or live-fire exercises at least once per year. The scenarios include a prompt-injected reviewer bot, fork-PR secret exfiltration, and an AI-generated malicious workflow file.                                                                                                                                                 | 3     |
...
* NIST Special Publication 800-204D: Strategies for Integrating Software
...
* OWASP Top 10 CI/CD Security Risks (10 risks): (1) CICD-SEC-01 Insufficient
  Flow Control Mechanisms, (2) CICD-SEC-02 Insufficient Identity and Access
  ...
  Insecure System Configuration, (8) CICD-SEC-08 Ungoverned Use of Third-Party
...
* GitHub Docs: Security hardening for GitHub Actions; Approving workflow runs
  ...
  protection rules and rule sets
