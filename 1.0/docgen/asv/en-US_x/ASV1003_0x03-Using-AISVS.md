...
The Artificial Intelligence Security Verification Standard (AISVS) defines security requirements for modern AI applications and services, focusing on aspects under the control of application developers.
...
Level 3 includes controls that are typically more difficult to implement or that apply only in specific situations. These often represent defense-in-depth measures or mitigations against niche, targeted, or high-complexity attacks.

## Scope of the AISVS

AISVS is intentionally narrow. It defines security requirements that are specific to AI and ML systems, or where general security controls have AI-specific nuances that warrant restating. It is not a standalone security program for an AI application. AISVS assumes that the underlying application, infrastructure, and organizational practices have already been verified against established general-purpose standards, and adds the AI-specific layer on top.

The following items are intentionally out of scope and are not duplicated in AISVS chapters:
...
When verifying an AI application against AISVS, the equivalent levels of those underlying standards should be verified in parallel. AISVS by itself is not a substitute for, and does not subsume, any of them.

## Alignment with ASVS Levels

AISVS levels are aligned with[ASVS](https://owasp.org/www-project-application-security-verification-standard/)levels. Verifying an AI application against AISVS Level_N_assumes the application has also been, or is being, verified against ASVS Level _N_The two standards are designed to be used together at matching levels:

| AISVS Level | Corresponding ASVS Level | Typical use                                                                                                                                 |
| :---------: | :----------------------: | ------------------------------------------------------------------------------------------------------------------------------------------- |
|      1      |            1             | Baseline security for any AI application that handles untrusted input or processes data of any sensitivity.                                 |
|      2      |            2             | AI applications that handle sensitive business data, regulated data, or operate in adversarial contexts.                                    |
|      3      |            3             | High-assurance AI applications such as those that handle life-safety decisions, critical infrastructure, or highly sensitive personal data. |

If an AISVS requirement appears to overlap with an ASVS requirement, the AISVS version is restated only because it includes AI-specific implementation details, attack surface considerations, or evidence that an auditor needs to evaluate differently. In all other cases, the ASVS requirement governs, and AISVS does not repeat it.

## Cross-References within AISVS

AISVS chapters are organized by control family rather than by attack type or component. As a result, defending against a given AI threat usually requires applying requirements from multiple chapters together. For example, defending against prompt injection in an agentic application combines requirements from C2 (input validation), C7 (output controls), C9 (agent action gating), C10 (MCP-specific controls), C11 (adversarial robustness), C13 (detection and logging), and C14 (human approval for high-impact actions).

Individual chapters and sections do not specify which other AISVS chapters address related concerns. When applying AISVS, treat the standard as a whole and consult Appendix D (AI Security Controls Inventory) for a cross-cutting view of where each defensive technique appears.
