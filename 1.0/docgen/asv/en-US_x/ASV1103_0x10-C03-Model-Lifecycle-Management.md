...
| 3.1.3 | Verify that model artifact signatures and integrity checksums are verified at deployment admission and on load, and that unsigned, tampered, or mismatched artifacts are rejected.                                                                                                          |   2   |
| 3.1.4 | Verify that lineage and dependency tracking maintain a dependency graph that enables identification of all consuming services and agents per environment (e.g., dev, staging, prod).                                                                                                        |   3   |
| 3.1.5 | Verify that the model origin integrity and trace records include the identity of an authorizing entity, training data checksums, validation test results with a pass/fail status, a signature fingerprint/certificate chain ID, a creation timestamp, and approved deployment environments. |   3   |
...
| 3.2.4 | Verify that deployment is automatically blocked when the safety evaluation results do not meet the thresholds defined for the model being deployed.                                                                                                                                                                                                                                             |   1   |
...
| 3.2.6 | Verify that all model changes (deployment, configuration, retirement) generate immutable audit records that include a timestamp, an authenticated actor identity, a change type, and before/after states, along with trace metadata (environment and consuming services/agents) and a model identifier (version/digest/signature).                                                              |   2   |
...
|   #   | Description                                                                                                                                                                                                           | Level |
| :---: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: |
| 3.5.1 | Verify that hosted model dependencies are inventoried with the provider, endpoint, provider-exposed model identifier, and version or release identifier when available, along with fallback or routing relationships. |   1   |
| 3.5.2 | Verify that changes to the provider model, version, or routing trigger a security re-evaluation before continued use in high-risk workflows.                                                                          |   2   |
| 3.5.3 | Verify that the logs record the exact hosted model identifier returned by the provider, or explicitly record that no such identifier was exposed.                                                                     |   2   |
...
Fine-tuning pipelines are high-privilege operations that can alter deployed model behavior at scale. Multi-stage pipelines compound this risk because a compromise at any intermediate stage produces a subtly altered artifact that subsequent stages accept. Reward models used in RLHF are ML artifacts that are subject to tampering, yet they are often treated as static infrastructure rather than versioned, validated components.

|   #   | Description                                                                                                                                                                                                                                                                               | Level |
| :---: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: |
| 3.6.1 | Verify that the reward models used in RLHF fine-tuning are versioned, cryptographically signed, and integrity-verified before use in a training run.                                                                                                                                      |   2   |
| 3.6.2 | Verify that starting a fine-tuning or retraining run requires authorization from a person who did not request the run (separation of duties).                                                                                                                                             |   2   |
...
| 3.6.4 | Verify that in multi-stage fine-tuning pipelines, the output of each stage is integrity-verified before the next stage consumes it.                                                                                                                                                       |   3   |
