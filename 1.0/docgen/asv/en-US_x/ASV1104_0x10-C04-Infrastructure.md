...
Isolate untrusted AI models in secure sandboxes and protect sensitive AI workloads with trusted execution environments (TEEs) and confidential computing technologies. For sandboxing tool and plugin execution within agentic workflows, see C9.3.
...
| 4.1.5 | Verify that confidential workloads run within a trusted execution environment (TEE) that provides hardware-enforced isolation, memory encryption, and integrity protection.                                                                                                                  |   3   |
...
|   #   | Description                                                                                                                                                                                                                                     | Level |
| :---: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: |
| 4.2.1 | Verify that, before workload execution, the AI accelerator integrity is validated using hardware-based attestation mechanisms (e.g., TPM, DRTM, or an equivalent).                                                                              |   2   |
| 4.2.2 | Verify that accelerator (GPU) memory is isolated across workloads through partitioning mechanisms, with memory sanitization between jobs.                                                                                                       |   2   |
| 4.2.3 | Verify that the AI accelerator firmware is version-pinned, signed, and attested at boot; unsigned or debug firmware is blocked.                                                                                                                 |   2   |
...
| 4.2.8 | Verify that accelerator telemetry (power draw, temperature, error correction, performance counters) is exported to centralized security monitoring and that alerts are raised for anomalies indicative of side channels or covert channels.     |   3   |
...
| 4.3.4 | Verify that on-device inference runtimes enforce process, memory, and file access isolation to prevent model dumping, debugging, or the extraction of intermediate embeddings and activations.                                                          |   3   |
| 4.3.5 | Verify that model weights and sensitive parameters stored locally are encrypted using hardware-backed key stores or secure enclaves (e.g., Android Keystore, iOS Secure Enclave, TPM/TEE), with keys that are inaccessible to user space.               |   3   |
