...
This appendix provides a concise inventory of every security control and defense technique referenced across the AISVS requirements. Controls are grouped by security control category so implementers can find all related defenses in one place, regardless of which AISVS chapter defines them.
...
| Cryptographically secure MCP session IDs (not used for authentication)                                     | 10.2.8          |

Common pitfalls: reusing end-user credentials for agent-to-agent calls; using MCP session IDs as authentication tokens; not rotating agent credentials after suspected compromise.
...
| Control / Technique                                                                         | Requirement IDs |
| ------------------------------------------------------------------------------------------- | --------------- |
| Access controls for AI resources (datasets, models, endpoints, vector collections, compute) | 5.2.1           |
...
| Fine-grained agent action authorization (tools, parameters, resources, data scope)          | 9.6.1           |
...
| Pre-execution policy constraint gates (deny rules, allowlists, budgets)                     | 9.7.1           |
...
| Agent PDP runtime isolation from the agent execution environment                            | 5.5.1           |
| Structured action descriptions for PDP (not raw agent reasoning context)                    | 5.5.2           |
...
Common pitfalls: granting broad OAuth scopes instead of the minimal required set; failing to re-evaluate authorization when the context changes mid-session; allowing model-generated output to override hard policy decisions.
...
Protect data in transit between services, agents, tools, and edge devices.
...
Common pitfalls: allowing plaintext interconnects in multi-tenant GPU clusters; using SSE over the public internet without TLS; not validating certificates on internal service calls.
...
Common pitfalls: hardcoded secrets in configuration files or container images; neglecting rotation schedules; storing MCP OAuth tokens in server state instead of validating them externally.
...
| Model signature and checksum verification during deployment and loading           | 3.1.3              |
...
| Agent persistent state integrity protection (MAC/signature, rejection on failure) | 9.4.6              |
...
| Per-request demonstration count limits within the context window                                                                              | 2.1.5           |
...
| Character set allowlisting for model prompt inputs                                                                                            | 2.1.3           |
...
| Adversarial perturbation detection for image/audio inputs                                                                                     | 2.4.2           |
...
Common pitfalls: validating only the text modality while ignoring the image/audio channels; relying solely on regex without semantic detection; not validating tool outputs before they re-enter the agent context.
...
| Removal of system prompt and backend data from explanations                                                                                   | 7.5.1           |
...
| Recursion depth and maximum concurrency / fan-out limits                                                                                                                                                                           | 9.1.1                |
...
| Per-tool CPU, memory, disk, egress, and execution time limits with fail-closed termination upon breach                                                                                                                             | 9.3.2                |
...
| Tool and plug-in sandboxing (container, VM, WASM, OS sandbox)       | 9.3.1           |
...
Common pitfalls: sharing infrastructure between dev and prod; granting containers more capabilities than needed; failing to restrict access to the cloud metadata service from AI workloads.

---

## AD.11 Network Segmentation & Egress Control

Control AI workload network boundaries, traffic flow, and outbound access.

| Control / Technique                                                                     | Requirement IDs |
| --------------------------------------------------------------------------------------- | --------------- |
| Default-deny network policies with explicit allowlists                                  | 4.3.1           |
| Network segmentation across dev / test / prod environments                              | 4.3.2, 3.4.1    |
| Separate IAM roles and security groups by environment, with no shared principals.       | 4.3.6           |
...
| MCP function invocation is restricted to statically defined allowlisted names           | 10.6.2          |
...
Common pitfalls: allowing AI workloads to reach cloud metadata services; not logging egress traffic for forensic analysis; failing to validate the Origin header, enabling DNS rebinding attacks.
...
| SBOM generation from automated builds                                            | ASVS v5 V15.1.2 / SCVS    |
...
Common pitfalls: failing to scan fine-tuning datasets for poisoning; not having rollback procedures when a compromised model is detected; treating AI BOMs as static documents instead of version-controlled artifacts.
...
| Safety evaluations with pass/fail thresholds prior to deployment                            | 3.2.3           |
| Automatic deployment block due to safety evaluation threshold failure                       | 3.2.4           |
...
| Atomic state restoration on rollback (weights, configuration, adapters, safety models)      | 3.3.2           |
| Development / test / production environment separation                                      | 3.4.1           |
| No shared infrastructure across environment boundaries                                      | 3.4.2           |
| Version control for all development artifacts (hyperparameters, scripts, prompts, policies) | 3.4.3           |
| Secure model artifact wiping and cryptographic erasure upon retirement                      | 3.5.1           |
...
Common pitfalls: not testing rollback procedures before they are needed; leaving retired model artifacts in serving caches; failing to propagate shutdown to downstream tools and MCP connections.
...
| Consent scope validation before model inference (operations and data subjects)   | 12.5.1          |
| Consent scope enforcement: refuse or downgrade the response before serving       | 12.5.2          |
...
| Federated training utility loss bound against the ε budget                       | 12.6.4          |
...
Test for and defend against evasion, data extraction, inversion, poisoning, and alignment-bypass attacks.

| Control / Technique                                                                                                                                                                                    | Requirement IDs |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------- |
| Refusal and safe-completion guardrails                                                                                                                                                                 | 11.1.1          |
| Red-team and jailbreak test suites (version-controlled)                                                                                                                                                | 11.1.2          |
| Automated harmful-content rating evaluation with regression detection                                                                                                                                  | 11.1.3          |
...
| Adversarial hardening configurations and procedures that are documented and reproducible                                                                                                               | 11.2.5          |
| Adversarially robust distillation — distill the teacher into the student using adversarial training so the student inherits robustness as well as accuracy (implementation example for 11.2.3)         | 11.2.3          |
| Certified robustness metrics tracked per model version, with degradation alerting                                                                                                                      | 11.2.4          |
...
| Model ensemble for evasion containment — route queries through independently trained models and flag disagreements beyond a threshold (implementation example for 11.2.2)                              | 11.2.2          |
| Output calibration and perturbation for privacy                                                                                                                                                        | 11.3.1          |
| Confidence obfuscation / output perturbation — return calibrated but deliberately imprecise confidence scores to hinder model extraction and membership inference (implementation example for 11.3.1)  | 11.3.1          |
| DP-SGD (differentially private training) with a documented epsilon                                                                                                                                     | 11.3.2          |
| PATE (Private Aggregation of Teacher Ensembles) — train the student model using noisy aggregation of teacher outputs so no individual training record is exposed (implementation example for 11.3.2)   | 11.3.2          |
...
| Statistical outlier detection and consistency scoring on external inputs                                                                                                                               | 11.6.1          |
...
| Self-modification reversibility and integrity verification enabling rollback to a known-good state                                                                                                     | 11.9.4          |
| Safety-violation feedback pipeline integrity, poisoning detection, and human review gates                                                                                                              | 11.9.6          |
| Data augmentation with perturbed inputs to improve training robustness                                                                                                                                 | 1.4.6           |
| RONI (Reject On Negative Influence) filtering — score the influence of each training sample and reject samples that degrade held-out performance beyond a threshold (implementation example for 1.4.2) | 1.4.2           |
...
| AI interaction logs exclude prompt and response content by default; content logging requires explicit opt-in and documented justification.                               | 13.1.2          |
...
| Self-modification logging is classified as a security event with what/when/by-whom/authorization details.                                                                | 11.9.3          |
...
| AI-specific forensic tools for model behavior investigations                                                                            | 13.5.2                 |
...
Common pitfalls: failing to correlate AI-specific events with broader SIEM alerts; treating model drift as a scheduled check rather than continuous monitoring; lacking AI-specific forensic tooling during incident response.

---

## AD.18 Explainability & Transparency

Enable human understanding of model decisions through interpretability artifacts and uncertainty quantification, with explanations sanitized to prevent leaking internal context.
...
| Confidence or uncertainty estimation for generated responses                                              | 7.2.1           |
| Automatic blocking or fallback when confidence falls below a defined threshold                            | 7.2.2           |
| Model output calibration to reduce overconfident predictions that are exploitable by membership inference | 11.3.1          |

Common pitfalls: providing explanations that expose system prompts or internal architecture; treating LLM-generated rationales as faithful descriptions of model internals; failing to calibrate uncertainty estimates so downstream gates can trust them.

---

## AD.19 Human Oversight & Approval Gates

Require human review and approval for high-impact, irreversible, or safety-critical actions, and provide reliable shutdown and graceful-degradation paths under human control. Effective human oversight requires four cooperating layers: a policy that classifies which actions are high-risk (C14.2), a runtime gate that blocks execution until approval is received (C9.2), kill-switch and graceful-degradation mechanisms to halt or constrain the system when needed (C14.1), and independent audit trails for both approvals (C13.6.4) and human-initiated overrides (C14.3). Each layer is independently verifiable; an approval gate without a policy is unenforceable, a policy without a runtime gate is unenforced, and either without audit trails is unattributable.
...
| High-impact intent verification with exact parameter binding and a quick expiration window                                                               | 9.2.3           |
| Fail-closed default action (block pending action) when human approval is not received within TTL                                                         | 14.2.2          |
| Any non-fail-closed TTL-expiry default that is explicitly authorized and classified as a high-risk policy decision requiring approval authority sign-off | 14.2.3          |
| Human review on anomaly detection                                                                                                                        | 11.6.3          |
| High-risk model quarantine with human review and sign-off                                                                                                | 6.1.3           |
| Post-condition outcome checking with containment for mismatch                                                                                            | 9.7.2           |
| Compensating actions and transactional rollback on failure                                                                                               | 9.2.4           |
| Manual kill switch to stop model inference and outputs                                                                                                   | 14.1.1          |
...
Common pitfalls: documenting a high-risk action policy that is never wired to a runtime gate; binding approval to a hash of parameters without binding to identity or context (replay across sessions); confirmation tokens without quick expiration; defaulting to fail-open when the approver does not respond, silently bypassing the gate; assuming an in-band kill switch will work against a compromised agent; implementing a kill switch but never exercising it, allowing it to atrophy until the moment it is needed.
