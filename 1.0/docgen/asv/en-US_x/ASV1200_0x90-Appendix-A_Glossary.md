...
This comprehensive glossary provides definitions of key AI, ML, and security terms used throughout the AISVS to ensure clarity and a shared understanding.

* Adapter – A lightweight module (e.g., LoRA, QLoRA) added to a pre-trained model to specialize its behavior for a specific task without modifying the original weights.

* Adversarial Example – An input deliberately crafted to cause an AI model to make a mistake, often by adding subtle perturbations that are imperceptible to humans.
...
* Adversarial Training – A training technique that augments the training data with adversarial examples to improve model robustness against perturbation attacks.
...
* AI BOM (AI Bill of Materials) – A structured record of all components in an AI system, including models, datasets, weights, hyperparameters, frameworks, and licenses. It may follow SPDX or CycloneDX formats. It is distinct from a traditional SBOM in that it covers model-specific artifacts beyond software dependencies. Also referred to as AIBOM or MBOM (Model Bill of Materials).
...
* Attribute-Based Access Control (ABAC) – An access control paradigm in which authorization decisions are based on attributes of the user, resource, action, and environment, and are evaluated at query time.
...
* Blue-Green Deployment – A deployment strategy that runs two identical production environments (blue and green), enabling instant rollback by switching traffic between them.
...
* Canary Deployment – A deployment strategy that gradually routes a small percentage of traffic to a new model version to detect issues before the full rollout.

* Cedar - An open-source policy language and evaluation engine for fine-grained permissions, originally created by Amazon. Used to implement ABAC for AI systems.
...
* Confidential Computing – A security paradigm that protects data in use by performing computation within hardware-enforced trusted execution environments, ensuring that code and data remain encrypted and isolated from the host.

* Confidential Inference – An inference service that runs AI models inside a trusted execution environment (TEE), ensuring that model weights and inference data remain encrypted, sealed, and protected from unauthorized access or tampering.
...
* Context Window - The maximum amount of text (measured in tokens) that a language model can process in a single inference call, including the system prompt, conversation history, retrieved documents, and tool outputs. The context window determines what information is available to the model at inference time and is a finite resource that can be depleted or manipulated by adversarial inputs.

* Counterfactual Explanation – An interpretability technique that explains a model decision by describing the minimal changes to input features required to change the prediction outcome.
...
* Data Leakage - Unintended disclosure of sensitive information through AI model outputs or behavior.
...
* Defensive Distillation - A training technique in which a model is trained on the soft probability outputs of another model to smooth decision boundaries and reduce susceptibility to adversarial perturbations.
...
* Explainability – The ability of an AI system to provide human-understandable reasons for its decisions and predictions, using techniques such as SHAP, LIME, attention maps, and counterfactual explanations. Also referred to as Explainable AI (XAI).
...
* Interval-Based Propagation – A formal verification technique that propagates bounds through neural network layers to certify that model predictions are robust within specified input perturbation ranges.
...
* JIT (Just-in-Time) Privileged Access — A security practice in which elevated permissions are granted only for a short, defined window when needed for a specific task and then automatically revoked afterward, minimizing exposure to standing privileges.
...
* KMS (Key Management Service) – A managed service for creating, storing, rotating, and managing access to the cryptographic keys used to protect data and assets.
...
* LIME (Local Interpretable Model-agnostic Explanations) – A technique for explaining the predictions of any machine learning classifier by approximating it locally with an interpretable model.
...
* Machine Unlearning – Techniques for removing the influence of specific training data from a trained model, supporting data subject deletion requests and regulatory compliance.
...
* Model Card – A document that provides standardized information about an AI model's performance, limitations, intended use, and ethical considerations to promote transparency and responsible AI development.

* Model Extraction – An attack in which an adversary repeatedly queries a target model to create a functionally similar copy without authorization. Also referred to as model stealing or model theft.
...
* mTLS (Mutual TLS) – A TLS configuration in which both the client and server authenticate each other using certificates, ensuring bidirectional identity verification for service-to-service communication.
...
* Non-repudiation – A security property ensuring that a party cannot credibly deny having performed an action. In AI systems, it is achieved through cryptographic signing of agent actions and audit log entries, enabling the attribution of decisions to specific principals.
...
* PII (Personally Identifiable Information) – Any information that can be used to identify, contact, or locate a specific individual, either alone or when combined with other data.
...
* Quantization – A post-training compression technique that reduces model weight precision (e.g., from 32-bit to 8-bit or 4-bit integers) to decrease memory footprint and inference latency. Quantization can change model behavior, so safety and robustness properties must be re-evaluated after deployment.
...
* RBAC (Role-Based Access Control) – An access control model in which permissions are assigned to roles rather than individual users, and users are granted access by being assigned to the appropriate roles.
...
* Reward Model – A machine learning model trained to predict human preference scores for AI outputs, used as a proxy reward signal in RLHF training pipelines. Because reward models are machine learning artifacts, they are subject to data poisoning attacks that can subvert alignment training outcomes.

* RLHF (Reinforcement Learning from Human Feedback) – A training technique in which a model is fine-tuned using human preference judgments as a reward signal to improve alignment with human values and safety requirements.
...
* Sandboxing – An isolation technique that confines a process or component to a controlled environment with restricted filesystem access, limited network egress, and restricted system call permissions. In AI systems, sandboxing is used to contain tool and plugin execution, AI workloads, and third-party model inference to prevent unauthorized host access or cross-tenant contamination.

* SBOM (Software Bill of Materials) – A formal record containing the details and supply-chain relationships of the software components used to build an application. See also AI BOM for model-specific artifacts.
...
* seccomp (Secure Computing Mode) – A Linux kernel feature that restricts the system calls a process can make, used to sandbox AI workloads and reduce the attack surface.
...
* SLSA (Supply-chain Levels for Software Artifacts) – A security framework that defines incremental levels of supply-chain integrity guarantees, from basic build-process documentation to fully reproducible, hermetically sealed builds with authenticated artifact provenance. Referenced by AISVS for AI model and artifact supply-chain control requirements.
...
* stdio (Standard Input/Output) – A process communication mechanism that uses standard input, output, and error streams. In MCP, it is used as a local-only transport restricted to single-process, same-machine communication.

* Strong Authentication – Authentication that resists credential theft and replay by requiring at least two factors (knowledge, possession, inherence) and phishing-resistant mechanisms such as FIDO2/WebAuthn, certificate-based service authentication, or short-lived tokens.
...
* Temperature Scaling – A post-hoc calibration technique that adjusts a model's output confidence scores to better reflect true prediction probabilities.
...
* TPM (Trusted Platform Module) – A dedicated hardware chip that provides cryptographic functions, including secure key generation, secure storage, and platform integrity measurement.
...
* Watermarking – Techniques for embedding imperceptible markers in AI-generated content or model weights to track origin, detect unauthorized copies, or identify AI-generated media.

* WORM (Write-Once-Read-Many) – A storage technology that prevents data from being modified or deleted after it is written, used for tamper-evident audit logs and backup protection.
