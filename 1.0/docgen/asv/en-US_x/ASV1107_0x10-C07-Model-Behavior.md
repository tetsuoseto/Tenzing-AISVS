...
|   #   | Description                                                                                                                                                                                                                                                                                    | Level |
| :---: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: |
| 7.1.1 | Verify that the application validates all model outputs against a strict schema (such as JSON Schema) and rejects any output that does not match.                                                                                                                                              |   1   |
| 7.1.2 | Verify that the system uses "stop sequences" or token limits to strictly terminate generation before it can overflow buffers or execute unintended commands.                                                                                                                                   |   1   |
| 7.1.3 | Verify that model outputs that cross a trust boundary into downstream interpreters (e.g., databases, shells, deserializers, template engines, browsers) are treated as untrusted input and processed using the corresponding safe APIs as defined in the OWASP ASVS v5 chapters V1.2 and V1.5. |   1   |
...
| 7.2.2 | Verify that the application automatically blocks answers or switches to a fallback message if the confidence score falls below a defined threshold.                                                                                                                                                                                                  |   2   |
...
| 7.2.5 | Verify that for responses classified as high-risk or high-impact by policy, the system performs an additional verification step via an independent mechanism, such as retrieval-based grounding against authoritative sources, deterministic rule-based validation, tool-based fact-checking, or consensus review by a separately provisioned model. |   3   |

---

## C7.3 Output Safety & Privacy Filtering

Technical controls to detect and remove bad content before it is shown to the user.
...
Ensure the user understands why the decision was made.

|   #   | Description                                                                                                                                               | Level |
| :---: | --------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: |
| 7.4.1 | Verify that the explanations provided to the user are sanitized to remove system prompts or backend data.                                                 |   1   |
...
Provide cryptographic provenance for synthetic media so downstream consumers can distinguish AI-generated content from authentic content.

|   #   | Description                                                                                                                | Level |
| :---: | -------------------------------------------------------------------------------------------------------------------------- | :---: |
| 7.5.1 | Verify that all generated media includes an invisible watermark or a cryptographic signature to prove it was AI-generated. |   3   |

---

## C7.6 Source Attribution & Citation Integrity

Ensure that RAG-grounded outputs are traceable to their source documents and that cited claims are verifiably supported by the retrieved content.
...
| 7.6.2 | Verify that RAG attributions are derived from retrieval metadata and are not generated by the model, ensuring that provenance cannot be fabricated. |   1   |
| 7.6.3 | Verify that each cited claim in a RAG-grounded response can be traced back to a specific retrieved chunk.                                           |   3   |
| 7.6.4 | Verify that the system detects and flags responses where claims are not supported by any retrieved content before the response is delivered.        |   3   |
