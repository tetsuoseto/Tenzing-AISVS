...
|   #    | Description                                                                                                                                                                                                                                                                                                               | Level |
| :----: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: |
| 2.1.1  | Verify that all external or derived inputs that may steer model behavior are treated as untrusted and are screened by a prompt-injection detection ruleset or classifier before being included in prompts or used to trigger actions.                                                                                     |   1   |
...
| 2.1.4  | Verify that prompts originating from third-party content (web pages, PDFs, emails) are sanitized in isolation (for example, by stripping instruction-like directives and neutralizing HTML, Markdown, and script content) before being concatenated into the main prompt.                                                 |   2   |
...
| 2.1.6  | Verify that prompt-injection screening respects user-specific attributes (e.g., age tier, authorization role, regional content-policy classification) through attribute-based rules resolved at request time, including the role or permission level of the calling agent.                                                |   2   |
...
| 2.1.8  | Verify that the instruction hierarchy is preserved across multi-step interactions and tool-augmented workflows, including prompt composition and intermediate outputs, so that user-controlled content cannot override system or developer instructions.                                                                  |   3   |
...
| 2.2.2 | Verify that inputs identified as adversarial by any detection mechanism are blocked from inclusion in prompts or from executing actions.                                                                                                                                                            |   1   |
| 2.2.3 | Verify that inputs that still contain suspicious encoding artifacts after normalization are rejected or flagged for review.                                                                                                                                                                         |   2   |
...
| 2.3.2 | Verify that prompt-content classification is evaluated for unsupported-language abuse, and that the identified gaps are mitigated through compensating controls such as language detection with rejection, conservative thresholds, or human review routing.            |   1   |
| 2.3.3 | Verify that inputs that violate policies are rejected so they do not propagate to downstream model or tool/MCP calls.                                                                                                                                                   |   1   |
| 2.3.4 | Verify that screening logs include classifier confidence scores and policy category tags, along with the applied stage (pre-prompt or post-response) and trace metadata (source, tool or MCP server, agent ID, session) for SOC correlation and future red-team replay. |   2   |

---

## C2.4 Multi-Modal Input Validation

AI systems that accept non-textual inputs (images, audio, video, files) face unique attack vectors in which malicious content can be embedded across modalities and then extracted into text that feeds the model’s context.

|   #   | Description                                                                                                                                                                                                                                                                                                                  | Level |
| :---: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: |
| 2.4.1 | Verify that text extracted from non-text inputs (e.g., image-to-text, speech-to-text) and hidden or embedded content (metadata, layers, alt text, comments) is treated as untrusted and screened in accordance with 2.1.1.                                                                                                   |   1   |
| 2.4.2 | Verify that image/audio inputs are checked for adversarial perturbations, steganographic payloads, or known attack patterns, and that detections trigger gating (block or degrade capabilities) before the model is used.                                                                                                    |   3   |
| 2.4.3 | Verify that cross-modal attack detection identifies coordinated attacks spanning multiple input types (e.g., steganographic payloads in images combined with prompt injection in text) using correlation rules and alert generation, and that confirmed detections are blocked or require HITL (human-in-the-loop) approval. |   3   |
